import os
from transformers import AutoModelForCausalLM, AutoTokenizer, get_scheduler,AdamW, Adafactor
import torch
...
def train(accelerator, config):
    ...
    # Create accelerator
    deepspeed_plugin = DeepSpeedPlugin(zero_stage=config["zero_stage"], 
    ...
    tokenizer = AutoTokenizer.from_pretrained(config['repo_name'], model_max_length=config['max_length'])
    ...
    model = AutoModelForCausalLM.from_pretrained(config["repo_name"], 
