...
# model init
model_args = dict(num_hidden_layers=n_layer, num_attention_heads=n_head, hidden_size=n_embd,intermediate_size=intermediate_size, block_size=block_size, num_key_value_heads=n_key_value_heads,
                   vocab_size=vocab_size, attention_dropout=dropout, activation_function=activation_function, rms_norm_eps=layer_norm_epsilon,use_cache= use_cache,pad_token_id=pad_token_id,
                   bos_token_id=bos_token_id, eos_token_id=eos_token_id, max_position_embeddings=n_positions, rope_theta=rope_theta,
                   sliding_window=sliding_window) # start with model_args from command line
...
if init_from == 'scratch':
    ...
    model = AutoModelForCausalLM.from_pretrained(repo_name).to(device)
